...
# 두산 로봇 워크스페이스 소싱 후 PATH 재정리
echo "🔧 PATH 재정리 중..."
export PATH=$(echo $PATH | tr ':' '\n' | grep -v '/home/ldj/noetic_ws' | tr '\n' ':' | sed 's/:$//')
export PATH="/opt/ros/humble/bin:$PATH"  # ROS2 bin 경로를 최우선으로 추가
...
echo "ros2 launch dsr_bringup2 dsr_bringup2_moveit.launch.py mode:=real model:=a0912 host:=192.168.137.100"
echo ""
